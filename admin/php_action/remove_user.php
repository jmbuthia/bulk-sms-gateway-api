...
//remove user company query
$sql_remove_user_company = "DELETE FROM users_company WHERE username = '{$username}'";
...
if($_SESSION['username'] == $username || $dbOperation->is_super_admin($username)){
	if($dbOperation->is_super_admin($username)){
		$validator['success'] = false;
		$validator['messages'] = "You cannot remove the System Administrator";
	}else {
		$validator['success'] = false;
	...
	try {
		...
		//delete user company
		$delete_user_company=$connect->query($sql_remove_user_company);
		
		//delete contacts
		//$delete_contacts=$connect->query($sql_remove_contacts);
		
		
		if (!$delete_user_result || !$delete_login_result || !$delete_user_rights || !$delete_user_company /* || !$delete_contacts */){
			/* Rollback */
			$validator['success'] = false;
