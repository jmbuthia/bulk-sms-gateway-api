...
if($_POST) {
	...
	if(($_SESSION['username'] == $username  && $active !=1) ||  ($dbOperation->is_super_admin($username) && $active !=1)){
		if($dbOperation->is_super_admin($username)){
			$validator['success'] = false;
			$validator['messages'] = "You cannot deactivate the System Administrator";
		}else {
			$validator['success'] = false;
		...
		if($dbOperation->is_phone_the_same($phone_before,$company_name)){
