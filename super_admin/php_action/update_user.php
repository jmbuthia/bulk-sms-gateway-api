...
if($_POST) {
	...
	//update user company
	$sql_update_user_company="UPDATE `users_company` SET `company_name`='$company' WHERE `username`='$username'";
	...
	if($_SESSION['username'] == $username && $active !=1){
		...
		if($dbOperation->is_phone_the_same($phone_before,$company)){
			...
			try {
				...
				//update user company
				$update_user_company_result=$connect->query($sql_update_user_company);
				
				
				if (!$update_user_result || !$update_contact_result || !$update_user_company_result){
					/* Rollback */
					$validator['success'] = false;
			...
			if(!$query || !$update_user_company_result) {
				$validator['success'] = true;
