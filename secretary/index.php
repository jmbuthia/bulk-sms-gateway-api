...
if (isset($_SESSION['category'])) {
	...
	if (strcmp($_SESSION['category'],"Secretary") != 0){
		...
		$errorMessage.=" Your session have been nullified. Please login as Secretary";
...
switch($action) {
	...
	case 'send_individual_sms':
		...
		if ($phones && $message) {
			...
			//Sending the sms
			$username=$_SESSION['username'];
			M_sender::post_async('http://ruemerc.co.ke/apps/smsgateway/message_sender/post_async_sms_individual.php',$contact_size,$message,$phone_list,$username);
	...
	case 'send_group_sms':
		...
		if ($groupids && $message) {
			...
			$username=$_SESSION['username'];
			M_sender::post_async('http://ruemerc.co.ke/apps/smsgateway/message_sender/post_async_sms.php',$group_size,$message,$groupid_list,$username);
