...
$sql = "SELECT DISTINCT scheduled_sms.`messageid`, scheduled_sms.`message`,
			 scheduled_sms.`scheduled_time`, scheduled_sms.`company_name` FROM 
			`scheduled_sms` INNER JOIN scheduled_sms_list ON 
			scheduled_sms.messageid=scheduled_sms_list.messageid WHERE
			 scheduled_sms_list.sent_by='$username' AND 
			scheduled_sms.`is_sent` = 0 AND 
			scheduled_sms.company_name='$company_name' ORDER BY 
			`scheduled_sms`.`scheduled_time`  ASC;";
